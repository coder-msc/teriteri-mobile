...
function moveUnderline(target) {
  if (target === 'B') {
    ...
    const offset = B.value.offsetLeft;
    -A.value.offsetLeft;
    ...
    <div id="view" v-show="visible" @click="AppOnClick">
      ...
      <div id="view-select-1">
        <div id="view-select">
          ...
          <div id="view-select-button-body">
            <div class="view-select-button" ref="A" @click="TabTo(0)" :style="{color:viewSelectColorA}"><span
                class="view-select-button-font">简介</span></div>
            <div class="view-select-button" ref="B" @click="TabTo(1)" :style="{color:viewSelectColorB}"><span
                class="view-select-button-font">评论</span></div>
          ...
          <div class="view-select-button-input-body">
            <div class="view-select-button-input" ref="viewSelectButtonInput" :class="{ 'expanded': isExpanded }">
              ...
              <div class="view-select-button-input-dan" @click="OnvViewSelectButtonInputClick">
                ...
                <el-icon size="6rem" color="#0264e7" v-if="!isExpanded" class="view-select-button-input-font-icon">
                  <Check/>
                </el-icon>
                <el-icon size="6rem" color="rgba(0, 0, 0, 0.2)" v-if="isExpanded"
                         class="view-select-button-input-font-icon">
                  <Close/>
                </el-icon>
              </div>
              <!--  控件--请输入文字  -->
              <div class="view-select-button-input-body-msg" @click="InputBarrage"><span
                  class="view-select-button-input-body-msgFont">{{ inputMsg }}</span></div>
            </div>
          </div>
        </div>
        <!--用户简介页面评论输入界面   -->
      ...
      <div :style="{'font-size':'4rem', 'z-index': '9999'}" v-show="InputDomState">
        <div id="inputDm" :style="{position: 'fixed', bottom: 0,width: '100%',height:'10rem'}">
          <van-icon @click="onClickComments" :color="inputDmInputState?'#0264e7':'#e9e9eb'" size="9rem"
                    class="inputDm-icon1" name="flag-o"/>
          <input type="text" ref="inputDmInput" @keyup.enter="OnClickSend" id="inputDm-input" v-model="barrage.text"
                 @click="OnClickinputDmInput" @focus="onInputFocus" @blur="onInputBlur" placeholder="点击输入框弹出键盘">
          ...
          <div class="inputDm-body">
            ...
            <div>
              ...
              <van-radio-group v-model="checkedLocation" direction="horizontal" checked-color="red">
                ...
                <van-radio name="scroll"
                           :style="{'--van-radio-label-color':checkedLocation=='scroll'?' #0264e7 ':' black '}">
                  滚动
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="bottom"
                           :style="{'--van-radio-label-color':checkedLocation=='bottom'?' #0264e7 ':' black '}">
                  底部
                  <template #icon="props">
                  </template>
                </van-radio>
            ...
            <div>
              ...
              <van-radio-group v-model="checkedColor" direction="horizontal" checked-color="red">
                <van-radio name="#337ecc">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#337ecc'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#337ecc'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="#79bbff">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#79bbff'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#79bbff'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="#529b2e">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#529b2e'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#529b2e'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                ...
                <van-radio name="#95d475">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#95d475'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#95d475'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="#b88230">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#b88230'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#b88230'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="#eebe77">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#eebe77'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#eebe77'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="#c45656">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#c45656'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#c45656'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="#f89898">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#f89898'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#f89898'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="#73767a">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#73767a'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#73767a'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="#b1b3b8">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#b1b3b8'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#b1b3b8'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="#000000">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#000000'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#000000'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
                <van-radio name="#FFFFFF">
                  <div class="inputDm-body-font-color"
                       :style="{'border': checkedColor=='#FFFFFF'?' 2px solid #0264e7':' 0px solid #0264e7',background:'#FFFFFF'}"></div>
                  <template #icon="props">
                  </template>
                </van-radio>
              </van-radio-group>
